  ...
  <script>
    ...
    window.addEventListener("DOMContentLoaded", () => {
      ...
      if (!student) {
        ...
        fetchExams(student.department, student.level);
      }
    });

    async function fetchExams(department, level) {
      try {
        const response = await fetch("https://cbt-system.onrender.com/api/exams/available", {
          method: "POST",
        ...
        data.forEach(course => {
          ...
          div.textContent = `${course.course} (${course.courseCode})`;
          div.addEventListener("click", () => {
            localStorage.setItem("courseTitle", course.course);
